...
def subprocess_fn(rank, args, temp_dir):
    ...
    # Print network summary on cpu
    # G = copy.deepcopy(args.G).eval().requires_grad_(False)
    # if rank == 0 and args.verbose:
    #     z = torch.empty([1, G.z_dim])
    #     c = torch.empty([1, G.c_dim])
    #     misc.print_module_summary(G, [z, c])

    # Print network summary on gpu
