...
@register_metric
def ppl2_zend(opts):
    ppl = perceptual_path_length.compute_ppl(opts, num_samples=50000, epsilon=1e-4, space='z', sampling='end', crop=False, batch_size=2)
    return dict(ppl2_zend=ppl)

@register_metric
def ppl2_zfull(opts):
    ppl = perceptual_path_length.compute_ppl(opts, num_samples=50000, epsilon=1e-4, space='z', sampling='full', crop=False, batch_size=2)
    return dict(ppl2_zfull=ppl)

@register_metric
def eqt50k_int(opts):
    opts.G_kwargs.update(force_fp32=True)
    del opts.G_kwargs.truncation_psi
