...
def compute_feature_stats_for_generator(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64, batch_gen=None, sfid=False, **stats_kwargs):
    ...
    while not stats.is_full():
        ...
        for _i in range(batch_size // batch_gen):
            ...
            # for fastGAN
            w = torch.randn([batch_gen, G.z_dim], device=opts.device)
            w = w.unsqueeze(1)
            
            if G.c_dim != 0:
                class_indices = np.random.RandomState(None).randint(low=0, high=G.c_dim, size=(batch_gen))
                class_indices = torch.from_numpy(class_indices).to(opts.device)
                c = torch.nn.functional.one_hot(class_indices, G.c_dim)
                c = c.to(opts.device)
                img = G.synthesis(w, c)
