...
class ProjectedDiscriminator(torch.nn.Module):
    ...
    def forward(self, x, c, return_out0_sm=False):
        logits = []
        out0_sm = []
        ...
        for bb_name, feat in self.feature_networks.items():
            ...
            # forward pass
            features, out0_scale_mixed = feat(x_n)
            out0_sm += out0_scale_mixed
            logits += self.discriminators[bb_name](features, c)
            # logits.append(self.discriminators[bb_name](features, c))
        
        if return_out0_sm == True:
            return logits, out0_sm
