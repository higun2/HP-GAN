...
"""Train a GAN using the techniques described in the paper HP-GAN."""
...
@click.option('--cfg',          help='Base configuration',                                      type=click.Choice(['stylegan2', 'fastgan', 'fastgan_lite']), required=True, default='fastgan_lite')
...
@click.option('--subset',       help='Train with only N images [default: all]', metavar='INT', type=int)
...
# BT loss & SC (support both lowercase and uppercase flags)
@click.option('--ft', '--FT', help='Apply barlow twins loss', metavar='BOOL',               type=bool, default=True, show_default=True)
@click.option('--dc', '--DC', help='Apply score consistency', metavar='BOOL',               type=bool, default=True, show_default=True)
...
def main(**kwargs):
    ...
    if opts.cfg == 'stylegan2':
        ...
        c.G_kwargs.synthesis_kwargs.lite = (opts.cfg == 'fastgan_lite')  # fastgan lite
    ...
    if subset is not None:
        ...
        if not 1 <= subset <= c.training_set_kwargs.max_size:
            raise click.ClickException(f'--subset must be between 1 and {c.training_set_kwargs.max_size}')
        desc += f'-subset{subset}'
        if subset < c.training_set_kwargs.max_size:
            c.training_set_kwargs.max_size = subset
    ...
    c.loss_kwargs.blur_init_sigma = 2   # Blur the images seen by the discriminator. // 0 : No blur   / 2 : blur
    c.loss_kwargs.blur_fade_kimg = 200  # 200k
    c.loss_kwargs.pl_weight = 0.0       # for fastgan generator. // 2.0 for stylegan generator

    # c.loss_kwargs.cls_weight = opts.cls_weight  # use classifier guidance only for superresolution training (i.e., with pretrained stem)
    # c.loss_kwargs.cls_model = 'deit_small_distilled_patch16_224'

    c.loss_weights = dnnlib.EasyDict()

    # Barlow-Twins head
    if opts.ft:
        print('### Apply Fake Twins ###')
        projector='128-512-512-512'
        lambd = 0.0051

        c.DHead_kwargs = None
        c.GHead_kwargs = dnnlib.EasyDict(class_name='training.ft_head.FT_Head', batch_size=c.batch_size, projector=projector, lambd=lambd, gpus=c.num_gpus)
        c.loss_weights.lw_ft = 0.02
    ...
    if opts.dc:
        print('### Apply Discriminator Consistency ###')
        c.loss_weights.lw_dc = 1.0
    else:
        c.loss_weights.lw_dc = 0.0
