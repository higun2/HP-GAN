...
def training_loop(
    ...
    DHead_kwargs            = None,     # Options for real SSL head.
    GHead_kwargs            = None,     # Options for fake SSL head.
    loss_weights            = {},       # Options for multiple loss weights.
    ...
    if num_gpus > 1:
        if DHead is not None:
            if rank==0:
                print('Apply sync batch normalization on DHead')
            DHead = torch.nn.SyncBatchNorm.convert_sync_batchnorm(DHead)
        if GHead is not None:
            if rank==0:
                print('Apply sync batch normalization on GHead')
            GHead = torch.nn.SyncBatchNorm.convert_sync_batchnorm(GHead)
    ...
    if (resume_pkl is not None) and (rank == 0):
        ...
        for name, module in [('G', G), ('D', D), ('G_ema', G_ema), ('DHead', DHead), ('GHead', GHead)]:
            if module is None:
                continue
    ...
    if rank == 0:
        print(f'Distributing SSL Heads across {num_gpus} GPUS...')
    if num_gpus > 1:
        if DHead is not None:
    ...
    # Setup SSL training phases.
    if rank == 0:
        print('Setting up SSL training phases...')
    ft_phases = dict()
    for name, module, opt_kwargs, reg_interval in [('GHead', GHead, G_opt_kwargs, G_reg_interval), ('DHead', DHead, D_opt_kwargs, D_reg_interval)]:
        if module is None:
            continue
        opt_kwargs.lr = opt_kwargs.lr
        opt = dnnlib.util.construct_class_by_name(module.parameters(), **opt_kwargs) # subclass of torch.optim.Optimizer
        ft_phases.update({name+'main': dnnlib.EasyDict(name=name+'main', module=module, opt=opt, interval=1)})
    ...
    while True:
        ...
        for phase, phase_gen_z, phase_gen_c in zip(phases, all_gen_z, all_gen_c):
            ...
            for real_img, real_c, gen_z, gen_c in zip(phase_real_img, phase_real_c, phase_gen_z, phase_gen_c):
                loss.accumulate_gradients(phase=phase.name, real_img=real_img, real_c=real_c, gen_z=gen_z, gen_c=gen_c, gain=phase.interval, cur_nimg=cur_nimg, ft_phases=ft_phases, **loss_weights)
        ...
        if (network_snapshot_ticks is not None) and (done or cur_tick % network_snapshot_ticks == 0):
            snapshot_data = dict(G=G, D=D, G_ema=G_ema, GHead=GHead, DHead=DHead, augment_pipe=augment_pipe, training_set_kwargs=dict(training_set_kwargs))
        ...
        if cur_tick and (snapshot_data is not None) and (len(metrics) > 0):
            ...
            if rank == 0:
                if 'fid50k_full' in stats_metrics and stats_metrics['fid50k_full'] < best_fid:
                    ...
                    snapshot_data['progress'] = best_fid
